...
export default class App extends Component<Props, State> {
        ...
        if (this.cacheState.adds > 0 || this.cacheState.moves > 0) {
            ...
            if (empty.length <= 1) {
                for (var i = 0; i < 4; i++) {
                    for (var j = 0; j < 3; j++) {
                        if (
                            this.cacheState.data[i][j].check(
                                this.cacheState.data[i][j + 1]
                            )
                        ) {
                            isEnd = false;
                            break;
        ...
        for (i = 0; i < 4; i++) {
            for (j = 1; j < 4; j++) {
                ...
                while (k - 1 >= 0 && this.cacheState.data[i][k - 1].isEmpty()) {
                    ...
                    this.cacheState.data[i][k].swap(
                        this.cacheState.data[i][k - 1]
                    );
                    k--;
                }
            }
        ...
        for (i = 0; i < 4; i++) {
            for (j = 0; j < 3; j++) {
                if (
                    ...
                    this.cacheState.data[i][j].combine(
                        this.cacheState.data[i][j + 1]
                    );
            ...
            for (j = 2; j >= 0; j--) {
                ...
                while (k + 1 <= 3 && this.cacheState.data[i][k + 1].isEmpty()) {
                    if (
                        ...
                        this.cacheState.moves++;
                    this.cacheState.data[i][k + 1].swap(
                        this.cacheState.data[i][k]
                    );
                    k++;
                }
            }
            ...
            for (i = 1; i < 4; i++) {
                ...
                while (k - 1 >= 0 && this.cacheState.data[k - 1][j].isEmpty()) {
                    if (
                        ...
                        this.cacheState.moves++;
                    this.cacheState.data[k][j].swap(
                        this.cacheState.data[k - 1][j]
                    );
                    k--;
                }
            }
            ...
            for (i = 2; i >= 0; i--) {
                ...
                while (k + 1 <= 3 && this.cacheState.data[k + 1][j].isEmpty()) {
                    if (
                        ...
                        this.cacheState.moves++;
                    this.cacheState.data[k + 1][j].swap(
                        this.cacheState.data[k][j]
                    );
                    k++;
                }
            }
    ...
    private addDown() {
        ...
        for (j = 0; j < 4; j++) {
            for (i = 3; i >= 1; i--) {
                if (
                    this.cacheState.data[i - 1][j].check(
                        this.cacheState.data[i][j]
                    )
                ) {
                    this.cacheState.data[i][j].combine(
                        this.cacheState.data[i - 1][j]
